...
\end{align*}
\end{theorem} 
\textbf{Key Insights.} In \Cref{th:main}, $\frac{\ip{G_l(x),G_l(x')}}w$ are the \emph{base kernels} and $\Pi_{l=1}^{d-1} \frac{\ip{G_l(x),G_l(x')}}w$ is a product of these base kernels and hence the name `Product of Kernels Theorem'.  The base kernels are essentially measuring the \emph{\textbf{correlation of the gates}}, which we show via our experiments is the key to `what is learnt in a DNN with ReLUs'. We now list the roles of the two networks, weights, depth and width.
